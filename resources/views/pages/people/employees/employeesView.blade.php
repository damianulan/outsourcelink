...
@endsection
@section('content')
<?php $dataView = Session::get('employeesDataView'); ?>
<main id="main-container">
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill h3 my-2">
                    {{ __('pages.employees') }} <small class="d-block d-sm-inline-block mt-2 mt-sm-0 font-size-base font-w400 text-muted">{{ __('pages.employees_d_view') }}</small>
                </h1>
                <nav class="flex-sm-00-auto ml-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-alt">
                        <li class="breadcrumb-item">{{ __('pages.employees') }}</li>
    ...
    <div class="content">
        @include('pages.people.employees.employeesNav');
        <!-- Basic -->
        <div class="block block-rounded">
            <div class="block-content block-content-full">
                <table id="employeeTable" class="table table-bordered table-striped table-vcenter js-dataTable-buttons js-table-checkable js-table-checkable-enabled">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center font-size-sm"><a href="{{ route('employees.edit', $employee->id) }}">{{ $employee->id }}</a></td>
                            <!-- end proprietary-->
                            @if (isset($dataView['fullname']))
                            <td class="font-w600 font-size-sm">
                                <a href="{{ route('employees.edit', $employee->id) }}">{{ $employee->firstname . ' ' . $employee->surname }}</a>
                            </td>
                            @endif
                            @if (isset($dataView['firstname']))
                            <td class="font-w600 font-size-sm">
                            ...
                            <td class="d-none d-sm-table-cell font-size-sm">
                                @foreach ($citizenships as $citizenship)
                                    @if ($citizenship->id == $employee->citizenships_id)
                                        {{ $citizenship->name }}
                                    @endif
                                @endforeach
                            </td>
                            ...
                            <td class="d-none d-sm-table-cell font-size-sm">
                                @foreach ($nations as $nation)
                                    @if ($nation->id == $employee->nations_id)
                                        {{ $nation->name }}
                                    @endif
                                @endforeach                            </td>
                            @endif
                            ...
                            <td class="d-none d-sm-table-cell font-size-sm">
                                {{ $employee->res_country . ', ' . $employee->res_postal . ' ' . $employee->res_place . ', ' . $employee->$res_street . ', ' . $employee->$res_flat . '.' }}
                            </td>
                            @endif
                            @if (isset($dataView['mailing_address']))
                            <td class="d-none d-sm-table-cell font-size-sm">
                                {{ $employee->mail_country . ', ' . $employee->mail_postal . ' ' . $employee->mail_place . ', ' . $employee->$mail_street . ', ' . $employee->$mail_flat . '.' }}
                            </td>
                            @endif
                            @if (isset($dataView['contact_numbers']))
                            <td class="d-none d-sm-table-cell font-size-sm">
                                {{$employee->phone . ', ' . $employee->email . ' | ' }} <a href="{{ route('employees.edit', $employee->id) }}">More >></a>
                            </td>
                            @endif
                            @if (isset($dataView['actions']))
                            <td>
                                <div class="btn-group">
                                    <a href="{{ route('employees.edit', $employee->id) }}"><button type="button" class="btn btn-sm btn-light js-tooltip-enabled" data-toggle="tooltip" title="edit_employee" data-original-title="Edit Client">
                                        <i class="fa fa-fw fa-pencil-alt"></i>
                                    </button></a>
                                    <form method="post" action="{{route('employees.destroy', $employee->id)}}">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-light js-tooltip-enabled" data-toggle="tooltip" title="delete_employee" data-original-title="Remove">
    ...
    <div class="modal" id="modal-block-normal" tabindex="-1" role="dialog" aria-labelledby="modal-block-normal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('employees.requestViews')}}" enctype="multipart/form-data">
                    @method('POST')
                    @csrf
